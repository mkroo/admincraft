...
| 역할 | 설명 | 주요 활동 |
|------|------|-----------|
| **시스템 관리자** | AdminCraft를 설치하고 운영하는 사람 | OIDC 인증 설정(Keycloak 등), OAS3 스펙 등록, AI 설정 (Admin Settings Page) |
| **기획자** | AI로 페이지를 만드는 비개발자 | 프롬프팅으로 페이지 생성, 갤러리에 공유 |
| **운영자** | 만들어진 페이지를 사용하는 비개발자 | 갤러리에서 구독, 데이터 조회/수정 |
| **개발자** | TSX를 직접 작성하는 사람 | 코드로 페이지 작성, 고급 커스터마이징 |
